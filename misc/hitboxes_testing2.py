...
def move_display(character: str, move: str, user: discord.User):
    ...
    char = character.lower().replace(' ', '-')
        # turns character name to lowercase, replaces all spaces with dash
    resp = requests.get(url=f'https://rivals.academy/library/{char}/data.json')
    data = resp.json()
        # accesses the json file for the appropriate character and stores it as a variable data
    ...
    for variant in data[move[0].lower() + move[1:].replace(' ', '')]:
        # uppends multiple varients (names) of move to a list
        ...
        for row in cur.execute(f"SELECT color, icon FROM characters WHERE name={character}"):
            icon = row[0]
            color = row[1]
        
        desc = f'Startup       : {move_startup}\nActive Frames : {move_active}\nEndlag        : {move_endlag}'
        
        con.close()
        
        embed = discord.Embed(description=desc)
        embed.set_image(url=db_image)
        title = f'{character} {move}'
        if name:
            title += f': {name}'
        embed.set_author(name=title, icon_url=info['icon'])
            # get icon from character db, doesn't need url
        embed.set_footer(text=f"Up-to-date as of Patch 1.3.1.2")
    ...
    if len(variants) > 1:
        for embed in variants:
            ...
            # from what I can tell this is just checking if there's multiple names for a move, forward tilt vs f tilt and so on
...
class Hitboxes(commands.Cog):
    ...
    moves = Literal[
        ...
        'Up Special/B', 'Neutral Special/B',
        'Down Special/B', 'Side Special/B '
    ]
    ...
    async def bandanadee(self, interaction: discord.Interaction, attack: moves):
        ...
        embed, view = move_display('bandanadee', attack, interaction.user)
    ...
    moves = Literal[
        ...
        'Neutral Air', 'Down Air', 'Up Air', 'Forward Air', 'Back Air',
        'Down Smash', 'Up Smash', 'Forward Smash', 
        'Up Special/B', 'Neutral Special/B (Flowmotion)',
        'Down Special/B', 'Side Special/B'
    ]
